    ...
    public class YoutubeConnector {
        ...
        public List<VideoInfo> search(String keywords) {
        ...
        private static List<VideoInfo> setItemsList(Iterator<SearchResult> iteratorSearchResults) {

            //temporary list to store the raw data from the returned results
            List<VideoInfo> tempSetItems = new ArrayList<>();
            ...
            while (iteratorSearchResults.hasNext()) {
                ...
                if (rId.getKind().equals("youtube#video")) {
                    ...
                    String videoChannel = singleVideo.getSnippet().getChannelId();
                    String description = singleVideo.getSnippet().getDescription();
                    String thumbnailURL = thumbnail.getUrl();
                    Bitmap bitmap = NetworkUtils.getBitmapFromURL(thumbnailURL);

                    //object of VideoItem class that can be added to array list
                    VideoInfo item = new VideoInfo(videoId, videoTitle, videoChannel,
                            description, thumbnailURL, bitmap);
